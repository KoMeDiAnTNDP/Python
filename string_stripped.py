...
def not_bad(s):
    ...
    last_index = s.find('bad') + 3

    if last_index < first_index or\
            first_index == -1 or last_index == -1:
        return s

    return s.replace(s[first_index:last_index], 'good')


def len_half_string(s):
    if len(s) % 2 == 0:
        return len(s) // 2
...
def front_back(a, b):
    a_half = len_half_string(a)
    b_half = len_half_string(b)

    return f'{a[:a_half]}{b[:b_half]}' \
        f'{a[a_half - len(a):]}{b[b_half - len(b):]}'
